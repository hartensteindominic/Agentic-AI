...
const { predictWeek, buildPersona, parseKeywords, runAgents } = require('../forecaster');
...
assert.equal(prediction.agents.length, 4);
assert.ok(prediction.agents.every((agent) => agent.assignment && agent.specialty));
...
assert.equal(runAgents(strained, prediction.days).length, 4);
